# ============================================================
# INVERTED-V | 62 cm | TWO-STEP STAIRCASE (TSSC)
# Winter (November–January) | 15-minute resolution
# Array-level PV modeling (no wire-level simulation)
# ============================================================

print("✅ Inverted-V | 62 cm | TSSC Winter simulation started")
...
# MAIN FUNCTION
# ------------------------------------------------------------
def run_inverted_v_62cm_tssc_winter():
    ...
    # ------------------ SHAPE: INVERTED-V
    irr_factor = 1.08          # dual orientation + sky view
    shape_temp_offset = -2.0   # chimney cooling

    # ------------------ SPACING: 62 cm (LOW CLEARANCE)
    spacing_offset = 4.0       # thermal penalty

    # ------------------ CONNECTION: TSSC
    tssc_gain = 1.10           # strongest mismatch mitigation
    ...
    # ------------------ OUTPUT FILE
    out_dir = Path(r"C:\Users\muthumaniraj\Documents\me research\me research\tssc\inverted-v")
    out_dir.mkdir(exist_ok=True)
    out_file = out_dir / "InvertedV_62cm_TSSC_Winter_15min.csv"
    ...
    with open(out_file, "w", newline="") as file:
        ...
        for i in range(len(timestamps) - 1):
            ...
            for minute in [0, 15, 30, 45]:
                ...
                # ------------------ CURRENT (TSSC applied)
                Isc_panel = (
                    Isc_stc * (G_eff / 1000)
                    * (1 + alpha_isc * delta_T)
                    * current_loss
                )
                Isc_array = Isc_panel * n_parallel * tssc_gain
...
if __name__ == "__main__":
    run_inverted_v_62cm_tssc_winter()
