...
print("🚀 PV DATASET GENERATION STARTED (Feb 2024 – Jan 2025)")
...
# ------------------------------------------------------------
# SEASON DEFINITION (FINAL – AS YOU APPROVED)
# ------------------------------------------------------------
SEASONS = {
    "Summer": ["04", "05","06"],
    "Autumn": ["07","08", "09"],
        ...
        for season, months in SEASONS.items():
            for shape, sconf in SHAPES.items():
                for spacing, soff in SPACINGS.items():
                    ...
                    out_dir = BASE_DIR / season / shape / f"{spacing}cm"
                    out_dir.mkdir(parents=True, exist_ok=True)
                    out_csv = out_dir / f"{shape}_{spacing}cm_{season}.csv"

                    with open(out_csv, "w", newline="") as sub:
                        sw = csv.writer(sub)
                        sw.writerow(HEADER)
                        ...
                        for i in range(len(timestamps) - 1):
                            ...
                            if dt1.strftime("%m") not in months:
                                continue
                            ...
                            for m in [0, 15, 30, 45]:
                                ...
                                row = [
                                    ...
                                    round(G_eff, 2), round(Tx, 2), round(T_cell, 2),
                                    round(Voc, 2), round(Isc, 2),
                                    round(Vm, 2), round(Im, 2), round(Pm, 2), FF
                                ]

                                sw.writerow(row)
                                mw.writerow(row)

    print("✅ DATASET COMPLETE — no missing months, no ambiguity")
